...
#define LINFO(tag, stream_expr)  LOG(INFO)  << "[" << tag << "] " << "[" << std::hex << std::this_thread::get_id() << "] " << std::dec << stream_expr << "\n"
#define LWARN(tag, stream_expr)  LOG(WARN)  << "[" << tag << "] " << "[" << std::hex << std::this_thread::get_id() << "] " << std::dec << stream_expr << "\n"
