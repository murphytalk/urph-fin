...
static const std::string asset_funds = {"Funds"};
static const std::string asset_stock = {"Stock"};
...
static StockPortfolio* prepare_stocks()
{
    ...
    builder->addTx(stock1_broker, stock1, asset_stock, stock1_buy_price, stock1_shares, fee, stock1_date);

    builder->add_stock(stock2, stock2_ccy);
    builder->prepare_tx_alloc(stock2, 1);
    builder->incr_counter(stock2);
    builder->addTx(stock2_broker, stock2, asset_stock, stock2_buy_price, stock2_shares, fee, stock2_date);

    builder->add_stock(stock3, stock3_ccy);
    builder->prepare_tx_alloc(stock3, 1);
    builder->incr_counter(stock3);
    builder->addTx(stock3_broker, stock3, asset_stock, stock3_buy_price, stock3_shares, fee, stock3_date);
...
static FundPortfolio* prepare_funds()
{
    ...
    builder->add_fund(funds1_broker, funds1, funds1_id, funds1_amt, funds1_capital, funds1_value, funds1_price, funds1_price, funds1_roi, funds1_date);
    builder->add_fund(funds2_broker, funds2, funds2_id, funds2_amt, funds2_capital, funds2_value, funds2_price, funds2_price, funds2_roi, funds2_date);
    builder->succeed();
    return funds;
}
